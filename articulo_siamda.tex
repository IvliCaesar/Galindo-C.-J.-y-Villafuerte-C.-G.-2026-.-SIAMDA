...
multilingual-MiniLM-L12-v2}, $d=384$), normalizado a norma unitaria. Tratando $x_i$ como una
señal discreta en $\mathbb{R}^d$ —indexada por las $384$ coordenadas del embedding, en el orden
en que el modelo las produce—, se aplica la transformada discreta de onduleta (DWT) con familia
Daubechies~4 y nivel de descomposición $J=3$ por defecto (acotado por
\texttt{pywt.dwt\_max\_level}$(d,\text{wavelet})$ si $d$ fuera más pequeño):
\[
...
\begin{warningBox}[title=Hueco metodológico: la DWT actúa sobre un eje sin orden natural]
A diferencia de una señal temporal o de una secuencia de tokens, las $384$ coordenadas de un
embedding de \emph{sentence-transformers} no tienen una noción intrínseca de adyacencia o
frecuencia: son ejes de un espacio de características aprendido, y su orden es un artefacto de
implementación del modelo, no una variable de la que el contenido semántico dependa
continuamente. La DWT, en cambio, sí depende esencialmente del orden: permutar las coordenadas
de $x_i$ antes de aplicar \texttt{wavedec} produce en general valores distintos de
$E^{\mathrm{apr}}_i$ y $E^{\mathrm{det}}_i$ para el \emph{mismo} contenido semántico. Por lo
tanto, la lectura ``aproximación = contenido estable, detalle = carga emocional local'' es una
interpretación tomada por préstamo del uso estándar de la DWT en señales con estructura
temporal/espacial genuina, no una propiedad derivada del significado de las coordenadas del
embedding. Esto no invalida $\hat\theta_i$ como estadístico reproducible —para un orden de
coordenadas fijo (el que produce el modelo), el índice es determinista y bien definido—, pero
sí significa que su interpretación causal como ``carga emocional'' no está justificada por la
matemática de la transformada en sí, sino que es una hipótesis de trabajo que requeriría
validación externa (p.\,ej.\ contra anotación humana de las mismas respuestas) para sostenerse.
\end{warningBox}
...
cota de Cramér--Rao acota inferiormente la varianza de cualquier estimador insesgado de $\mu$:
\[
...
muestral es el estimador insesgado de varianza mínima de $\mu$ \emph{para todo} $N$ (Lehmann--Scheffé),
con $\mathrm{Var}(\bar\theta)=\sigma^2/N$ exactamente. Es decir, bajo ese modelo la eficiencia es
$1$ por construcción, no por mérito del pipeline NLP--wavelet: no hay ningún estimador
alternativo de $\mu$ que pudiera ganarle a la media muestral. La comparación que hace el sistema
—$\mathrm{CRB}_\mu=\hat\sigma^2/N$ contra $\widehat{\mathrm{Var}}(\bar\theta)$ estimada por
\emph{bootstrap sobre la misma muestra}— es, en el límite de $N$ grande, una comparación entre
dos maneras de estimar la \emph{misma} cantidad ($\sigma^2/N$) a partir de los \emph{mismos}
datos: la fórmula normal con $\hat\sigma^2$ plug-in, y el remuestreo. Que ambas coincidan (EES
$\approx 1$) es entonces, en gran medida, una prueba de consistencia numérica entre bootstrap y
la fórmula asintótica de la varianza de la media —algo que ocurre para casi cualquier muestra
razonablemente grande, independientemente de qué tan bien el pipeline de embeddings y wavelets
capture sentimiento real—, y \emph{no} es evidencia de que ``el pipeline NLP-wavelet extrae la
máxima información posible del texto'', como sugiere el texto de interpretación de la interfaz
(\texttt{interpretar\_crb} en \texttt{interpretacion.py}). Verificar eso último exigiría comparar
$\hat\theta$ contra una pipeline de extracción de características alternativa (o contra
anotación humana) sobre el mismo texto, no solo verificar que la media muestral de $\hat\theta$
es un buen estimador de sí misma.
\end{warningBox}
